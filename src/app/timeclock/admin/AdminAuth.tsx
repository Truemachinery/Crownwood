...
import { loginTimeclockAdmin } from "@/app/actions/timeclock";
...
export function AdminAuth() {
    ...
    const handleSubmit = async () => {
        ...
        setLoading(true);
        const res = await loginTimeclockAdmin(pin);
        if (res.success) {
            router.refresh();
        } else {
        ...
        <div className="bg-industrial border border-white/10 rounded-3xl p-8 shadow-2xl">
            ...
            <div className="flex justify-center gap-4 mb-8">
                {[0, 1, 2, 3].map((i) => (
                    <div
                        key={i}
                        className={`w-4 h-4 rounded-full transition-all duration-300 ${pin.length > i ? "bg-red-500 scale-110 shadow-[0_0_15px_rgba(239,68,68,0.5)]" : "bg-white/10"}`}
                    />
                ))}
            </div>
            ...
            <div className="grid grid-cols-3 gap-3">
                ...
                <button
                    ...
                    className="h-16 bg-red-500 hover:bg-red-600 disabled:bg-white/10 disabled:text-white/30 text-white rounded-2xl font-mono text-xs font-bold uppercase tracking-widest transition-colors flex items-center justify-center"
                >
